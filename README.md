...
Estos archivos se consiguen en las release oficiales publicadas junto con el `.jar`.
...
También, dentro de `\target\surefire-reports`, se encuentran los resultados de los tests. Por ejemplo :
...
- A veces requería hacer modificaciones en otras clases
- Es difícil ser dogmático, pues a veces es necesario cambiar los casos de prueba porque se cambia la idea del diseño de lo que se entiende de los requerimientos
- La fase azul no tiene mucho sentido muchas veces. Cuando el código funciona, muchas veces ya está acabado, y puede ser contraproducente con el tiempo invertido tratar de que se vea más profesional. Quizás falta una definición aceptable de cuánto es suficientemente profesional, pues puede quedar al criterio de cada persona
- Encuentro difícil seguir el TDD con GitFlow, ya que inicialmente quería hacer varias ramas `feature/x`; pero al ir añadiendo casos de pruebas de manera incremental, terminaba programando diferentes features a la vez para que funcione un solo caso de prueba. La verdad es que terminé todos los requerimientos en una sola feature
- Adaptarse a pensar en TDD cuesta un poco. Igualmente, a medida que voy avanzando, me surgen varias dudas de cómo se aplica correctamente en cada caso
- Encuentro que es muy útil para enfocarse en lo que uno tiene que hacer. El método es un poco `greedy` o `local`, en el sentido de que tienes que ir un paso a la vez añadiendo características (este paso sería el ciclo), y luego añadir más casos de prueba, y añadir la siguiente cacterística. Por lo general, uno se abruma pensando en todas las posibilidades, pero así se siente más liviano
- Es más robusto contra fallas. A medida que completo todos los requerimientos para una función, luego puedo usar esa función en otra, y sé que está completamente bien; rara vez requiere alguna modificación. Solo requiriría una en caso de que uno se encuentre con una situación no prevista en los requerimentos o casos de pruebas. Pero, complementado bien con hacer buenas pruebas con técnicas de casja blanca y negra, debería ser más robusto aún
- Siento que el desarrollo fue más rápido de lo esperado. Por el punto anterior, no tuve que hacer tanto retrabajo. Me tarde más en configurar el CI/CD (no se pedía en la entrega; fue por cuenta propia)

### Pregunta 3

- Sí me gustaría seguir usando TDD, con el fin de hacerlo mejor la siguiente vez (en aplicarlo), y porque siento que tiene varias ventajas:
